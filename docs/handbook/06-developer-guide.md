...
metaphor migration generate Invoice subscription
metaphor migration run
...
(`subscription` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed subscription          # run Rust seeders
metaphor migration generate-seeds subscription  # emit SQL seed files
```
