...
metaphor make entity Vendor --module subscription
...
metaphor schema schema validate subscription

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate subscription --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor subscription
...
> `subscription` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate subscription`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> subscription` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration subscription --database-url …`).
4. Regenerate code: `metaphor schema schema generate subscription --target all --force`.
