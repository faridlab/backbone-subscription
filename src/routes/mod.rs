...
/// Create stateless routes for the Subscription module.
///
...
///
/// ```ignore
/// let module = SubscriptionModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::SubscriptionModule) -> Router<()> {
    Router::new()
        .merge(create_subscription_routes(module.subscription_service.clone()))
        .merge(create_subscription_billing_run_routes(module.subscription_billing_run_service.clone()))
        .merge(create_subscription_plan_routes(module.subscription_plan_service.clone()))
        .merge(create_subscription_plan_line_routes(module.subscription_plan_line_service.clone()))
}

/// Read-only routes for the Subscription module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_subscription_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_subscription_routes(module: &crate::SubscriptionModule) -> Router<()> {
    Router::new()
        .merge(create_subscription_read_routes(module.subscription_service.clone()))
        .merge(create_subscription_billing_run_read_routes(module.subscription_billing_run_service.clone()))
        .merge(create_subscription_plan_read_routes(module.subscription_plan_service.clone()))
        .merge(create_subscription_plan_line_read_routes(module.subscription_plan_line_service.clone()))
}

/// Get all routes (stateless) for the Subscription module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::SubscriptionModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Subscription module.
///
...
pub fn create_combined_routes(module: &crate::SubscriptionModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Subscription module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::SubscriptionModule) -> Router<AppState> {
    Router::new()
