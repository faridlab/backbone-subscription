...
async fn seed_due_subscription(pool: &PgPool) -> (Uuid, Uuid, Uuid, Uuid) {
    let (company, customer, item, revenue_acct, ar_acct) = (
    ...
    sqlx::query(
        r#"INSERT INTO subscription.subscription_plans
             (id, plan_code, name, company_id, billing_cycle, billing_day, currency,
              receivable_account_id, price, status)
           VALUES ($1, $2, 'Pro Monthly', $3, 'monthly', 1, 'IDR', $4, 100000, 'active')"#,
    )
    .bind(plan_id).bind(format!("PRO-{}", &Uuid::new_v4().simple().to_string()[..8]))
    .bind(company).bind(ar_acct)
    ...
    sqlx::query(
        r#"INSERT INTO subscription.subscriptions
             (id, subscription_number, company_id, customer_id, plan_id, status,
              started_at, current_period_start, current_period_end, next_billing_date, currency)
           VALUES ($1, $2, $3, $4, $5, 'active', $6, $6, $7, $7, 'IDR')"#,
    )
    .bind(sub_id)
    .bind(format!("SUB-{}", &Uuid::new_v4().simple().to_string()[..8]))
    .bind(company).bind(customer).bind(plan_id)
    .bind(last_month).bind(today)
    .execute(pool).await.unwrap();

    (sub_id, company, customer, plan_id)
}
...
async fn cadence_stages_invoice_due_and_relay_creates_billing_invoice() {
    ...
    let (sub_id, company, customer, plan_id) = seed_due_subscription(&pool).await;
    let today = chrono::Utc::now().date_naive();
    ...
    assert_eq!(due.company_id, company);
